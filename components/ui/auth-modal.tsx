...
const PROVIDER_META: Record<ProviderId, { label: string; icon: React.ReactNode }> = {
...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const [providers, setProviders] = React.useState<ProviderId[] | null>(null)
    ...
    getProviders()
      .then((available) => {
        ...
        const ids = Object.keys(available ?? {}).filter(
          (id): id is ProviderId => id === "github" || id === "google"
        )
        setProviders(ids)
      })
      .catch((err) => {
        ...
        console.error(err)
        setProviders([])
        setError("Couldn't load sign-in options. Refresh and try again.")
      })
  ...
  const handleSocialAuth = async (provider: ProviderId) => {
    setIsLoading(true)
    setError(null)
    try {
